...
# file modification date with a previously saved timestamp file. If the
...
function compare_timestamp ()
{
    ...
    then
        ...
        then
            ...
            # TODO: If there are no changes to the
            # files StaticDownloadFiles-modified.txt,
            # ExcludeDownloadFiles-modified.txt, and
            # StaticUpdateFiles-modified.txt, then the recursive download
            # of some configuration files could also be skipped. Then
            # the comparison of the timestamps should be done by the
            # function recursive_download instead of this function.
        fi
    fi
    rm -f "${pathname}.timestamp"
    return 0
