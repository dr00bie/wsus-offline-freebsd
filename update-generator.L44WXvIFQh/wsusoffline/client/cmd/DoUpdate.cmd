...
set WSUSOFFLINE_VERSION=11.7.2
title %~n0 %*
...
for %%i in (/updatecpp /instmssl /instdotnet35 /instpsh /instdotnet4 /instwmf /instmsse /updatetsc /skipieinst /skipdefs /skipdynamic /all /excludestatics /seconly /verify /autoreboot /shutdown /showlog /showdismprogress /monitoron /instmsi) do (
...
if %DOTNET35_VER_MAJOR% LSS %DOTNET35_VER_TARGET_MAJOR% (
  ...
  call :Log "Warning: Missing Windows PowerShell 2.0 prerequisite .NET Framework 3.5 SP1"
  goto SkipPShInst
)
...
if "%PSH_TARGET_ID%"=="" (
  ...
  call :Log "Warning: Environment variable PSH_TARGET_ID not set"
  goto SkipPShInst
)
%CSCRIPT_PATH% //Nologo //B //E:vbs ListInstalledUpdateIds.vbs
if exist "%TEMP%\InstalledUpdateIds.txt" (
...
if exist "%TEMP%\UpdatesToInstall.txt" (
  ...
  call :Log "Warning: Windows PowerShell 2.0 installation file (kb%PSH_TARGET_ID%) not found"
  goto SkipPShInst
)
set REBOOT_REQUIRED=1
:SkipPShInst
...
if "%RECALL_REQUIRED%"=="1" (
  if "%BOOT_MODE%"=="/autoreboot" (
    ...
    if "%USERNAME%" NEQ "WOUTempAdmin" (
      ...
      call PrepareRecall.cmd "%~f0" %VERIFY_MODE% %SKIP_IEINST% %UPDATE_CPP% %INSTALL_MSSL% %INSTALL_DOTNET35% %INSTALL_DOTNET4% %INSTALL_PSH% %INSTALL_WMF% %INSTALL_MSSE% %SKIP_DEFS% %UPDATE_TSC% %INSTALL_OFV% %INSTALL_MSI% %BOOT_MODE% %FINISH_MODE% %SHOW_LOG% %DISM_MODE% %MONITOR_ON% %LIST_MODE_IDS% %LIST_MODE_UPDATES% %SKIP_DYNAMIC%
    )
    if exist %SystemRoot%\System32\bcdedit.exe (
      echo Adjusting boot sequence for next reboot...
      %SystemRoot%\System32\bcdedit.exe /bootsequence {current}
