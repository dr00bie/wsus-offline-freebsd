...
function calculate_static_updates ()
{
    ...
    #
    # Testing these files beforehand may prevent unnecessary warnings,
    ...
    case "${name}" in
        ...
        dotnet)
            ...
            # specified languages from the command line.
            require_file \
                "../static/StaticDownloadLinks-${name}-${arch}-${lang}.txt" \
            || return 0
        ;;
...
function calculate_dynamic_office_updates ()
{
    ...
    #
    # But maybe this table is also needed for the detection of missing
    # updates.
    mkdir -p "../client/ofc"
    while read -r line
    do
...
function seconly_safety_guard ()
{
    ...
    local current_date=""                     # in ISO format: 2017-08-08
    local -i day_of_week="0"                  # as integer 1..7, with Monday=1
    local patchday_this_month=""              # for example 2017-08-08
    local -i patchday_this_month_seconds="0"  # seconds since 1970-01-01

    # Note: The variable "${day_of_month}" should get the values as zero
    # padded strings, to construct the full date in ISO format. Therefore,
    # the C-style loop "for (start;end;increment)" cannot be used at
    # this point.
    ...
    do
        ...
        then
            ...
            patchday_this_month_seconds="$(date -d "${current_date}" '+%s')"
        fi
    done
    ...
    #
    # GNU date understands relative date specifications in plain English
    # like "yesterday", "last week" or "last month". To get the last
    # month, we use the 15th of this month and go back to "last month":

    local last_month=""
    last_month="$(date -d "${this_month}-15 last month" '+%Y-%m')"
    ...
    do
        ...
        then
            ...
            patchday_last_month_seconds="$(date -d "${current_date}" '+%s')"
        fi
    done
    ...
    do
        ...
        then
            log_warning_message "The configuration file ${current_file} was modified on ${modification_date}, which is before the last official patch day on ${last_patchday}."
            misconfiguration="1"
        fi
    done
